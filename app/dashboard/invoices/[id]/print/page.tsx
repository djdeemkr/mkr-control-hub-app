...
export default async function InvoicePrintPage({
  ...
  if (userErr) {
    console.error("PRINT: auth error", userErr);
    redirect("/login");
  }
  ...
  if (invErr || !invoice) {
    console.error("PRINT: invoice fetch error", invErr);
    notFound();
  }
  ...
  if (payErr) console.error("PRINT: payments fetch error", payErr);
    ...
    <div style={{ padding: 24, color: "#111", fontFamily: "Arial, sans-serif" }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        <div className="noprint" style={{ marginBottom: 16 }}>
          {/* button uses window only in browser click, safe */}
          <button
